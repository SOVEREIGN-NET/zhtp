...
pub struct ZhtpUnifiedServer {
    // Network listeners - DEPRECATED: TCP/UDP being replaced by QUIC-only
    tcp_listener: Option<Arc<TcpListener>>,  // TO BE REMOVED
    udp_socket: Option<Arc<UdpSocket>>,      // TO BE REMOVED
    
    // QUIC-native protocol (required, not optional)
...
impl ZhtpUnifiedServer {
    ...
    pub async fn new_with_peer_notification(
        ...
        Ok(Self {
            tcp_listener: None,  // DEPRECATED: Will be removed in Phase 2
            udp_socket: None,    // DEPRECATED: Will be removed in Phase 2
            quic_mesh: quic_arc,
            quic_handler,
            http_router,
            mesh_router,
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        // DEPRECATED: TCP binding code kept for reference (will be removed)
        ...
        info!(" ⚠️  DEPRECATED: TCP listener bound on port {} (legacy support only)", self.port);
        
        // DEPRECATED: UDP socket binding (kept for backward compatibility)
        info!(" ⚠️  DEPRECATED: Binding UDP socket on {}...", bind_addr);
        ...
        info!(" ⚠️  DEPRECATED: UDP socket bound on port {} (legacy support)", self.port);
        
        // Set UDP socket in mesh router (temporary - will use QUIC instead)
        self.mesh_router.set_udp_socket(udp_socket_arc).await;
        info!(" ⚠️  DEPRECATED: UDP socket configured in mesh router");
        ...
        *self.is_running.write().await = true;
        ...
        // DEPRECATED: Start TCP listener for HTTP API (legacy support)
        self.start_tcp_listener().await?;
        info!(" ⚠️  DEPRECATED: TCP listener started (use native QUIC clients instead)");
        
        // DEPRECATED: Start UDP listener for mesh protocol (legacy support)
        self.start_udp_listener().await?;
        info!(" ⚠️  DEPRECATED: UDP listener started (use QUIC streams instead)");
    ...
    async fn start_tcp_listener(&self) -> Result<()> {
        ...
        // ❌ REMOVED: bootstrap_router - using server_id directly
        let server_id = self.server_id; // ✅ ADD: Pass server_id for bootstrap
        let is_running = self.is_running.clone();
            ...
            while *is_running.read().await {
                match listener.accept().await {
                    Ok((stream, addr)) => {
                        ...
                        // ❌ REMOVED: bootstrap_router clone - pass server_id directly
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = Self::handle_tcp_connection(
                                ...
                                wifi_router,
                                bluetooth_router,
                                server_id, // ✅ CHANGED: Pass server_id instead of bootstrap_router
                            ).await {
    ...
    async fn handle_tcp_connection(
        ...
        server_id: Uuid, // ✅ CHANGED: Pass server_id instead of BootstrapRouter
    ) -> Result<()> {
        ...
        match protocol {
            ...
            IncomingProtocol::Bootstrap | IncomingProtocol::Unknown => {
                ...
                // ✅ REFACTORED: Use lib-network bootstrap handler
    ...
    async fn start_udp_listener(&self) -> Result<()> {
        ...
        let mesh_router = Arc::new(self.mesh_router.clone());
        // ❌ REMOVED: bootstrap_router - using lib-network bootstrap
        let server_id = self.server_id; // ✅ ADD: server_id for bootstrap
        let http_port = self.port; // ✅ ADD: HTTP port for bootstrap response
        let zhtp_port = self.port; // ✅ ADD: ZHTP port for bootstrap response
        let is_running = self.is_running.clone();
